...
def CC_matrix_calculator(intens0, fname_output):
    ...
    #savfilename = Path + "ACC_training_CC_matrix.npz"
    #np.savez(savfilename, CC_models=CC_models,logCC_models=logCC_models, STD_models=STD_models,\
    # aveCC_models=aveCC_models)

    h5 = h5py.File(fname_output, 'w')
    h5['CC_models'] = CC_models
    h5['logCC_models'] = logCC_models
    h5['STD_models'] = STD_models
    h5['aveCC_models'] = aveCC_models
    h5.close()
...
if __name__ == '__main__':
    ...
    if (n_class == 0):
        h5 = h5py.File(Path + fname_inten, 'r')
        intens0 = h5['intens'][:]
        h5.close()
